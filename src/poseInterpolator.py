...
from tfx import transformations
import numpy as np
import math
...
def linear_pose_interp(start_A, end_B, T):
    ...
    ** "Roll" or twisting of the arm is not taken into account in this calculation. Separate interpolations have to be done for the roll angle
    ...
    A = array(start_A[:3]); B = array(end_B[:3])        # Performs linear interpolation first
    ...
    QA = Rotation.RPY(ra, pa, ya).GetQuaternion()  # Quaternion representation of start and end points
    QB = Rotation.RPY(rb, pb, yb).GetQuaternion()

    if (ra == rb and pa == pb and ya == yb):
        track['rot'] = QA             # List of quaternion [w x y z]

    else:
        q = transformations.quaternion_slerp(QA, QB, T)
        track['rot'] = [q[3], q[0], q[1], q[2]]    # List of quaternion [w x y z]
